...
//     		exit;
    	...
    	\$result=\$m->alias('g')->join('LEFT JOIN syh_attr_news i ON i.attr_id = g.id')->join('LEFT JOIN syh_news r ON r.id = i.news_id')->join('LEFT JOIN syh_column f ON f.id = r.nv_id')->join('LEFT JOIN syh_model m ON m.id = f.column_type')->field(\$field)->limit("$limit")->order('r.$order $by')->where(\$data)->where(\$where)->where("news_dell=0")->select();
// 		    	dump(\$result);
// 		    	exit;
		...
		if (strpos(\$modlu, "mobile")==''){//如果url中不存在mobile（不区分大小写
			...
			foreach(\$result as \$k3 => \$v3){
     		\$result[\$k3]['url'] = /*'__APP__'.*/'/'.\$v3['model_table'].'/'.detail.'/'.'id'.'/'.\$v3['id'].'.html';
     		}
		}else {
    		//手机站url
			foreach(\$result as \$k3 => \$v3){
     		\$result[\$k3]['url'] = /*'__APP__'.*/'/'.'mobile'.'/'.\$v3['model_table'].'/'.detail.'/'.'id'.'/'.\$v3['id'].'.html';
     		}
		}
...
//     	exit;
		...
		$str="<img src='__APP__/Dongqinmanage/Verify/code?w={$width}&h={$height}' onclick='this.src=this.src+\"?\"+Math.randon'/>";
...
// 		exit;
		...
		if (strpos(\$modlu, "mobile")==''){//如果url中不存在mobile（不区分大小写）
			...
			foreach(\$_nav_m as \$k3 => \$v3){
				if(\$v3['column_link']==1){
					\$_nav_m[\$k3]['url'] = /*'__APP__'.*/'/'.\$v3['column_url'].'.html';
				}
			
				if(\$v3['column_link']==2){
					\$_nav_m[\$k3]['url'] = /*'__APP__'.*/'/'.\$v3['column_ename'].'.html';
				}
			
				if(\$v3['column_link']==0){
					\$_nav_m[\$k3]['url'] = /*'__APP__'.*/'/'.\$v3['url'].'/'.group.'/'.'id'.'/'.\$v3['id'].'.html';
				}
     		}
		}else {
			//栏目url
...
//     		exit;
    	...
    	\$result=\$m->alias('g')->join('LEFT JOIN syh_column i ON i.id = g.nv_id')->join('LEFT JOIN syh_model r ON r.id = i.column_type')->field(\$field)->where(\$where)->where("news_dell=0")->relation(true)->limit("$limit")->order('$order $by')->select();
