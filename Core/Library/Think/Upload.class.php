...
class Upload {
    ...
    public function upload($files='') {
        ...
        /* 逐个检测并上传文件 */
        $info    =  array();
        if(function_exists('finfo_open')){//die("7777");
            $finfo   =  finfo_open ( FILEINFO_MIME_TYPE );
        }
        // 对上传文件数组信息处理
        $files   =  $this->dealFiles($files);//var_dump($files);die("111");
        foreach ($files as $key => $file) {
            ...
            /* 保存文件 并记录保存成功的文件 */
            if ($this->uploader->save($file,$this->replace)) {//die("111");
				...
				if(in_array($ext, array('gif','jpg','jpeg','bmp','png','swf'))) {
					$imginfo = getimagesize($file['tmp_name']);
					if(!$imginfo){//die("111");
						$tmp='./Uploads'.$file['savepath'].$file['savename'];
						//echo $tmp;die("222");
						if(!file_exists($tmp)){
							move_uploaded_file($file['tmp_name'],$tmp);
						}
    ...
    private function setDriver($driver = null, $config = null){
		...
		//var_dump($class);var_dump($config);die();
