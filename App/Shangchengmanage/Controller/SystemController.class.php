...
// 		exit;
    ...
    public function moban() {
		$dir_home='./Public/Home/';
		if (is_dir($dir_home)) {
			if ($dh = opendir($dir_home)) {
				while (($file = readdir($dh)) !== false) {
					if($file!='.'&&$file!='..'&&filetype($dir_home . $file)=='dir'){
						$qtmoban[$file]['name']=$file;
						if($file==C('DEFAULT_THEME__HOME')){
							$qtmoban[$file]['checked']='checked="checked"';
						}
					}
				}
				closedir($dh);
