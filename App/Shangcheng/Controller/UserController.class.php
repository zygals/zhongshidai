...
class UserController extends Controller {
	...
	public function keyongjifen(){
		if(session('bh')){
			...
			if(I('post.pageIndex')){
				...
				$ks=$num*$page;
				$keyong=D('User_jifen')->where(array('user_login_bh'=>$bh,'jifen_type'=>1))->order('id desc')->limit($ks.','.$num)->select();
				...
				if($keyong){
					foreach($keyong as $kk=>$kv){
						...
						$html.="<tr>
									<td>".date('Y-m-d H:i:s',$kv['jifen_date'])."</td>
									<td>".$ly."</td>
									<td>".$jf."</td>
									<td>".$kv['jifen_shengyu']."</td>
								</tr>";
					}//var_dump($kv);die();
	...
	public function daiyongjifen(){
		if(session('bh')){
			...
			if(I('post.pageIndex')){
				$page=I('post.pageIndex')-1;
				$ks=$num*$page;
				$daiyong=D('User_jifen')->where(array('user_login_bh'=>$bh,'jifen_type'=>2))->order('id desc')->limit($ks.','.$num)->select();
				if($daiyong){
					foreach($daiyong as $dk=>$dv){
						...
						$html.="<tr>
									<td>".date('Y-m-d H:i:s',$dv['jifen_date'])."</td>
									<td>".$ly."</td>
									<td>".$jf."</td>
									<td>".$dv['jifen_shengyu']."</td>
								</tr>";
					}
					echo json_encode(array('code'=>1,'data_info'=>$html));
					exit;
	...
	public function fenxiangjifen(){
		if(session('bh')){
			...
			if(I('post.pageIndex')){
				$page=I('post.pageIndex')-1;
				$ks=$num*$page;
				$fenxiang=D('User_jifen')->where(array('user_login_bh'=>$bh,'jifen_type'=>3))->order('id desc')->limit($ks.','.$num)->select();
				if($fenxiang){
					foreach($fenxiang as $fk=>$fv){
						...
						$html.="<tr>
									<td>".date('Y-m-d H:i:s',$fv['jifen_date'])."</td>
									<td>".$ly."</td>
									<td>".$jf."</td>
									<td>".$fv['jifen_shengyu']."</td>
								</tr>";
					}
					echo json_encode(array('code'=>1,'data_info'=>$html));
					exit;
	...
	public function zengsong(){
		if(session('bh')){
			...
			if(I('post.pageIndex')){
				$page=I('post.pageIndex')-1;
				$ks=$num*$page;
				$fenxiang=D('User_jifen')->where(array('user_login_bh'=>$bh,'jifen_type'=>10))->order('id desc')->limit($ks.','.$num)->select();
				if($fenxiang){
					foreach($fenxiang as $fk=>$fv){
						if($fv['jifen_laiyuan']){
							if($fv['jifen_yuanyin']==1){
								$ly='系统转入'.$fv['user_login_bh'];
								$jf=$fv['jifen_num'];
							}
	...
	public function baodanjifen(){
		...
		if(session('bh')){
			...
			if(I('post.pageIndex')){
				$page=I('post.pageIndex')-1;
				$ks=$num*$page;
				$baodan=D('User_jifen')->where(array('user_login_bh'=>$bh,'jifen_type'=>4))->order('id desc')->limit($ks.','.$num)->select();
				if($baodan){
		...
		if(strlen($user['user_zfpass'])>30){
				...
				if($jg1){
	               ...
	               // $userid = I('post.proid');
					...
					echo json_encode(array('code'=>1,'msg'=>'报单成功','url'=> '/u.html'));
					exit;
				}else{
	...
	public function chongzhiwr(){
		...
		$tixian->add($data);
		if($tixian->create()){	
			$this->success('充值成功，请等待审核', 'chongzhi');;	
		}else{
			$this->error('充值失败','chongzhi');
		}	
	}
	...
	public function PatrioticPoint(){
		if(session('bh')){
			...
			if(I('post.pageIndex')){
				$page=I('post.pageIndex')-1;
				$ks=$num*$page;
				$fenxiang=D('User_jifen')->where(array('user_login_bh'=>$bh,'jifen_type'=>5))->order('id desc')->limit($ks.','.$num)->select();
				if($fenxiang){
					foreach($fenxiang as $fk=>$fv){
						if($fv['jifen_laiyuan']){
						...
						$html.="<tr>
									<td>".date('Y-m-d',$fv['jifen_date'])."</td>
									<td>".$ly."</td>
									<td>".$jf."</td>
									<td>".$fv['jifen_shengyu']."</td>
								</tr>";
	...
	public function MaintenanceCost(){
		if(session('bh')){
			...
			if(I('post.pageIndex')){
				$page=I('post.pageIndex')-1;
				$ks=$num*$page;
				$fenxiang=D('User_jifen')->where(array('user_login_bh'=>$bh,'jifen_type'=>6))->order('id desc')->limit($ks.','.$num)->select();
				if($fenxiang){
					foreach($fenxiang as $fk=>$fv){
						if($fv['jifen_laiyuan']){
							if($fv['jifen_laiyuan']=='fh'){
	...
	public function BenevolencePoint(){
		if(session('bh')){
			...
			if(I('post.pageIndex')){
				$page=I('post.pageIndex')-1;
				$ks=$num*$page;
				$fenxiang=D('User_jifen')->where(array('user_login_bh'=>$bh,'jifen_type'=>7))->order('id desc')->limit($ks.','.$num)->select();
				if($fenxiang){
					foreach($fenxiang as $fk=>$fv){
						if($fv['jifen_laiyuan']){
							if($fv['jifen_laiyuan']=='fh'){
