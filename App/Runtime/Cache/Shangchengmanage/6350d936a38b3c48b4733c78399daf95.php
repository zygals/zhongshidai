...
<script src="/App/Shangchengmanage/View/Default/js/common.js" type="text/javascript"></script>
 <script type="text/javascript">
  function clear_del()
  {
   if(confirm("确定要删除数据吗？"))
   {
	subform('/shangchengmanage/message/delall');
   }
  }
 </script>
<table width="100%" height="31px" border="0" cellpadding="0" cellspacing="0" class="left_topbg" id="table2">
      <tr>
        <td height="31"><div class="titlebt">站内信息发送</div></td>
      </tr>
    </table>
<div class="main">
    <div class="operate">
        <div class="left">
            <input type="button" onclick="window.open('/shangchengmanage/message/add','main')" target="main" class="btn_blue" value="添加">
        </div>
    </div>
    <div class="list">    
    <form action="<?php echo U(GROUP_NAME.'/Member/delAll');?>" method="post" id="form_do" name="form_do">
        <table width="100%">
			...
			<?php if(is_array($vlist)): $i = 0; $__LIST__ = $vlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                <td width="20%"><?php echo ($v["gb_title"]); ?></td>
                <td width="50%"><p style="width:719px; height:39px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;line-height:39px"><?php echo ($v["gb_content"]); ?></p></td>
				<td width="20%"><?php echo (date('Y-m-d H:i:s',$v["gb_addtime"])); ?></td>
                <td width="10%">
                    <a href="/shangchengmanage/message/edit/id/<?php echo ($v["id"]); ?>">编辑</a>
					<a href="/shangchengmanage/message/del/id/<?php echo ($v["id"]); ?>">删除</a>
				</td>
