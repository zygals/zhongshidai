...
<div class="main">
    <div class="operate">
        <div class="left">
            <input type="button" onclick="window.open('/shangchengmanage/help/add','main')" target="main" class="btn_blue" value="添加">
        </div>
    </div>
    <div class="list">
        <table width="100%">
			...
			<?php if(is_array($vlist)): $i = 0; $__LIST__ = $vlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                <td><?php echo ($v["id"]); ?></td>
                <td><?php echo getCateName($v[fid]);?></td>
                <td><?php echo ($v["title"]); ?></td>
                <td><?php echo ($v["paixu"]); ?></td>
                <td><?php if($v['is_show'] == 1): ?>显示<?php else: ?>不显示<?php endif; ?></td>
                <td>
                    <a href="/shangchengmanage/help/edit/id/<?php echo ($v["id"]); ?>">编辑</a>
                    <a href="/shangchengmanage/help/del/id/<?php echo ($v["id"]); ?>" onclick="return confirm('是否确定删除?')">删除</a>
				</td>
